...
if (isset($_POST['btnAdd']) && !empty($_POST['btnAdd'])) {
   ...
   // echo "affected rows :" . mysqli_num_rows($result);
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<script type=\"text/javascript\">window.alert('Location Already Exists!');window.location.href = 'location_add_details.php';</script>";
    } else {
        $cstatus = "active";
       // echo "inserting city" . $sid;
        $q = "insert into tbl_city (city_name,state_id,city_status) values('".$_POST['txtCityName']."',$sid,'$cstatus')";
        mysqli_query($conn, $q);
        // echo "<script>swal('Done.!', 'Location Added Successfully.!', 'success');</script>";
        echo "<script type=\"text/javascript\">window.alert('Location Added Successfully.');window.location.href = 'location_view_details.php';</script>";
    }
