                            ...
                            <fieldset>
                                ...
                                <div class="form-actions">
                                    ...
                                    if (isset($_POST['btnAdd'])) {
                                        ...
                                        $u = "SELECT * FROM tbl_subject WHERE subject_name='" . $nm . "' ";
                                        $v = mysqli_query($conn, $u);
                                        $counttt = mysqli_fetch_row($v);
                                        if ($counttt > 0) {
                                            echo "<script type=\"text/javascript\">window.alert('Subject Already Exists!');window.location.href = 'subject_add_details.php';</script>";
                                        } else {
                                                $q = $conn->prepare("insert into tbl_subject (subject_name,subject_status) values(?,?)");
                                                $q->bind_param("ss", $nm, $a);
                                                $q->execute();
                                                echo "<script>swal('Done.!', 'Subject Added  Successfully.!', 'success');</script>";
                                        }
                                    }
                                    ?>
